...
本项目包含一套 Kubernetes YAML 配置文件，用于部署一个由 BungeeCord 代理的 Minecraft 服务器集群。该集群包含两个后端的 PaperMC 服务器实例：一个作为 "lobby" (大厅/主城)，另一个作为 "new" (新服务器/其他游戏模式)。

## 目录结构 (假设所有文件在同一目录)

- `00-namespace.yaml`: 定义 Kubernetes 命名空间。
- `01-minecraft-statefulset.yaml`: 定义 "lobby" PaperMC 服务器的 StatefulSet。
- `02-minecraft-headless-service.yaml`: 为 "lobby" PaperMC 服务器定义 Headless Service。
- `03-bungeecord-configmap.yaml`: 包含 BungeeCord 的 `config.yml` 配置文件。
- `04-bungeecord-deployment.yaml`: 定义 BungeeCord 代理服务器的 Deployment。
- `05-bungeecord-service.yaml`: 为 BungeeCord 代理定义 LoadBalancer Service，以便外部访问。
- `06-mc-new-server-statefulset.yaml`: 定义 "new" PaperMC 服务器的 StatefulSet。
- `07-mc-new-headless-service.yaml`: 为 "new" PaperMC 服务器定义 Headless Service。

## 先决条件

1.  **Kubernetes 集群**: 您需要一个正在运行的 Kubernetes 集群。例如，可以使用 Docker Desktop 内置的 Kubernetes, Minikube, Kind, 或者云提供商的 Kubernetes 服务。
2.  **`kubectl` 命令行工具**: 用于与您的 Kubernetes 集群进行交互。
3.  **Docker 镜像**:
    * **PaperMC 镜像**: 您需要为 PaperMC 服务器构建一个或多个 Docker 镜像。
        * 在 `01-minecraft-statefulset.yaml` 中，`lobby` 服务器使用的镜像 (例如 `spec.template.spec.containers[0].image`) 需要内置 `server.properties`，其中 `server-port` 设置为 `25535`，`online-mode` 设置为 `false`。
        * 在 `06-mc-new-server-statefulset.yaml` 中，`new` 服务器使用的镜像需要内置 `server.properties`，其中 `server-port` 设置为 `25565`，`online-mode` 设置为 `false`。
        * 这两个服务器可以使用相同的基础 PaperMC 镜像，但其启动端口和可能的其他配置（如世界名称）需要在镜像层面或通过启动脚本/环境变量（如果镜像支持）进行区分。**如果它们使用完全相同的镜像且该镜像硬编码了一个端口，则无法按此配置运行在不同端口。**
    * **BungeeCord 镜像**: 您需要一个 BungeeCord 的 Docker 镜像。在 `04-bungeecord-deployment.yaml` 中，`spec.template.spec.containers[0].image` 需要指向这个镜像。

## 配置说明

在部署之前，您可能需要检查并修改以下 YAML 文件中的一些关键配置：

* **`01-minecraft-statefulset.yaml` (Lobby 服务器)**:
    * `spec.template.spec.containers[0].image`: 替换为您为 `lobby` 服务器构建的 PaperMC 镜像名称和标签。
...
* **`03-bungeecord-configmap.yaml` (BungeeCord 配置)**:
    * `data.config.yml`:
...
* **`04-bungeecord-deployment.yaml` (BungeeCord 部署)**:
    * `spec.template.spec.containers[0].image`: 替换为您的 BungeeCord 镜像名称和标签。
    * `resources`: 根据需要调整 CPU 和内存。
* **`06-mc-new-server-statefulset.yaml` (New 服务器)**:
    * `spec.template.spec.containers[0].image`: 替换为您为 `new` 服务器构建的 PaperMC 镜像名称和标签。
...
* **`07-mc-new-headless-service.yaml` (New 服务)**:
...
2.  **部署所有其他资源**:
    ...
    ```bash
    # 部署 Lobby 服务器
    kubectl apply -f 01-minecraft-statefulset.yaml -n minecraft-server
    kubectl apply -f 02-minecraft-headless-service.yaml -n minecraft-server
...
3.  **（可选）重启 BungeeCord Pod**: 如果您在部署 BungeeCord 后修改了 `03-bungeecord-configmap.yaml`，您需要重启 BungeeCord Pod 来加载新的配置：
    ```bash
...
* **增加 PaperMC 服务器实例**:
    * 修改对应 StatefulSet (`01-...yaml` 或 `06-...yaml`) 中的 `spec.replicas` 字段。
    * `kubectl apply -f <statefulset_yaml_file> -n minecraft-server`
    * **注意**: 简单增加副本数会创建基于相同镜像和配置（但有独立存储）的新 Pod。您还需要在 BungeeCord `config.yml` 中为这些新 Pod 定义服务器条目。
* **增加 BungeeCord 代理实例**:
    * 修改 `04-bungeecord-deployment.yaml` 中的 `spec.replicas` 字段。
    * `kubectl apply -f 04-bungeecord-deployment.yaml -n minecraft-server`
